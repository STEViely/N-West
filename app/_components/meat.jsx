...
export default function page() {
  const info = [
    {
      id: 1,
      title: "1. ลดแสงสะท้อนและตัดแสง UV",
      content:
  ...
  return (
    <div>
      <p className="text-white text-[16px] md:text-[22px] 2xl:text-[28px]  mt-4">กระจกมองข้างของรถยนต์ที่มีสีฟ้า (Blue Side Mirrors) เป็นกระจกที่ถูกออกแบบมาเพื่อเพิ่มความปลอดภัยและความสะดวกสบายในการขับขี่</p>
      {info.map((el, index) => (
        <MeatContainer
          key={el.id}
          title={el.title}
